...
1. Read `REWRITE_TASKS.md` if it exists — that's current ground truth, not
   ...
   delegation" section and every file in `.claude/rules/` relevant to the
   task at hand.
...
4. For genuinely independent work, say so and note that
   backend-agui-developer and a2a-integrator should run with
   `isolation: worktree` if dispatched concurrently.
